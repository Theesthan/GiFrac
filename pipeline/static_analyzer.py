...
class StaticAnalyzer:
    ...
    def _run_pylint(self, path: Path) -> List[Issue]:
              r = subprocess.run(["pylint", "--output-format=json", "--recursive=y", str(path)],
    ...
    def _run_bandit(self, path: Path) -> List[Issue]:
              r = subprocess.run(["bandit", "-r", str(path), "-f", "json", "-q"],
                                                            capture_output=True, text=True)
              try:
                            raw = json.loads(r.stdout or "{}")
except json.JSONDecodeError:
    ...
    def _pylint_score(self, path: Path) -> float:
              r = subprocess.run(["pylint", "--recursive=y", str(path)],
                                                            capture_output=True, text=True)
              for line in r.stdout.splitlines():
